...
import { deletedownloadHistoryAPI, getDownnloadHistoryAPI } from '../services/allAPI'
...
function History() {
  ...
  const getHistory=async()=>{
    try{
      const result=await getDownnloadHistoryAPI()
      console.log(result);
          ...
          <div className='col-md-4' key={index}>
            <Paper elevation={3} sx={{my:5, p:5, textAlign: 'center'}}>
              <div className='d-flex align-item-center justify-content-evenly'>
                <h6>Review At :{item?.timeStamp} <br />item</h6>
                <button onClick={()=>handleRemove(item?.id)} className='btn text-danger fs-4 ms-5'><MdDelete />
